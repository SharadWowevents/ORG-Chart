...
export const Admin = ({ company, updateCompany, onDeleteCompany, onChangePassword, onViewChart }) => {
  ...
  const renderPersonEditor = (path, person, color, titlePlaceholder, onRemove) => (
    <div className="repeat-row" style={{ '--card-accent': color }}>
      ...
      <div className="repeat-row-main" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        
        {/* LEFT SIDE: Avatar + Inputs */}
        <div style={{ display: 'flex', gap: '12px', flex: '1 1 250px' }}>
          <label className="avatar avatar--sm avatar-upload" style={{ flexShrink: 0, ...(person.photoUrl ? {} : { background: color }) }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', flex: 1 }}>
            <input type="text" className="rr-name" value={person.name} onChange={e => updateField(`${path}.name`, e.target.value)} placeholder="Full name" style={{ flex: '1 1 120px', margin: 0, minWidth: '100px' }} />
            <input type="text" className="rr-title" value={person.title} onChange={e => updateField(`${path}.title`, e.target.value)} placeholder={titlePlaceholder} style={{ flex: '1 1 120px', margin: 0, minWidth: '100px' }} />
          </div>
        </div>
        
        {/* RIGHT SIDE: Action Buttons */}
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center', paddingTop: '4px' }}>
          <button className="btn btn-tiny btn-ghost" onClick={() => toggleKra(person.id)}>
             {expandedKra[person.id] ? "Hide KRA/KPI" : "KRA / KPI"}
          </button>
        ...
        {company.departments.map((dept, di) => {
          ...
          return (
            <div key={dept.id} className="dept-editor" style={{ '--card-accent': color }}>
              <div className="dept-editor-head" style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px', flexWrap: 'wrap' }}>
                 <input type="text" className="rr-name" value={dept.name} onChange={e => updateField(`departments.${di}.name`, e.target.value)} placeholder="Department Name" style={{ flex: '1 1 200px', margin: 0 }} />
              ...
              <div className="dept-editor-body">
                ...
                <div className="dept-editor-label" style={{ flexWrap: 'wrap', gap: '8px' }}>Managers
                ...
                {dept.managers.map((mgr, mi) => (
                  <div key={mgr.id} className="manager-block">
                    ...
                    <div className="team-block">
                      <div className="dept-editor-label dept-editor-label--sm" style={{ flexWrap: 'wrap', gap: '8px' }}>Team members
                        <button className="btn btn-ghost btn-tiny" onClick={() => {
                          const clone = structuredClone(company);
